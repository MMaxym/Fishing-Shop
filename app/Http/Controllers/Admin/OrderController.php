...
class OrderController extends Controller
{
    ...
    public function filter(Request $request)
    {
        ...
        if ($discount === 'null') {
            $orders->whereNull('discount_id');
        }
        elseif (!is_null($discount) && $discount !== '') {
            $orders->where('discount_id', $discount);
        }

        if ($request->filled('dateFrom')) {
            $orders->whereDate('created_at', '>=', $request->dateFrom);
        }

        if ($request->filled('dateTo')) {
            $orders->whereDate('created_at', '<=', $request->dateTo);
        }
    ...
    public function pdfExport(Request $request)
    {
        ...
        if ($request->filled('shippingMethod')) {
            $filteredOrder->where('shipping_method_id', $request->input('shippingMethod'));
        }

        if ($request->filled('paymentMethod')) {
            $filteredOrder->where('payment_method_id', $request->input('paymentMethod'));
        }

        if ($request->filled('discount')) {
            $discount = $request->input('discount');
            if ($discount === 'null') {
                $filteredOrder->whereNull('discount_id');
            } else {
                $filteredOrder->where('discount_id', $discount);
            }
        }

        if ($request->filled('status')) {
            $filteredOrder->where('status', $request->input('status'));
        }

        if ($request->filled('dateFrom') && $request->filled('dateTo')) {
            $filteredOrder->whereBetween('created_at', [
                Carbon::parse($request->input('dateFrom'))->startOfDay(),
                Carbon::parse($request->input('dateTo'))->endOfDay(),
            ]);
        }

        $orders = $filteredOrder->with(['user', 'paymentMethod', 'shippingMethod', 'discount'])
            ->get([
                'id', 'user_id', 'payment_method_id', 'shipping_method_id', 'discount_id',
